<template>
  <div class="contents md-elevation-1" style="padding: 0px 15px 1px 15px; margin:0px auto 50px;">
      ...
      <div class="md-layout" style="flex: 1 1 100%;	align-items: center;">
        ...
        >
          ...
          <md-input
            ...
            @keyup.native.enter="getContents()"
          ></md-input>
    ...
    <md-empty-state
      v-if="count < 1 && Object.keys(chips).length > 0"
    ...
    <div class="pager-container">
      <md-button
        ...
        @click="set_pager('first')"
        v-if="pager.length > 1 && page > 1"
      >
        <md-icon>first_page</md-icon>
      </md-button>
      <md-button
        class="md-icon-button"
        @click="set_pager('prev')"
        v-if="pager.length > 1 && page > 1"
      >
      ...
      <md-button
        class="md-icon-button"
        @click="set_pager('next')"
        v-if="pager.length > 1 && page < pager.length"
      >
        <md-icon>chevron_right</md-icon>
      </md-button>
      <md-button
        class="md-icon-button"
        @click="set_pager('last')"
        v-if="pager.length > 1 && page < pager.length"
      >
...
export default {
  ...
  methods: {
    getContents() {
      ...
      Object.keys(selected).map(function(Key) {
        ...
        if (val.length) {
          ...
          url +=
            (!first_query ? "&" : "") + "parameter[" + Key + "]=" + selectedTid;
      ...
      fetch("http://api.ed808.com/latin/contents?" + url)
    ...
    chips_delete(i) {
      //callback for delete button in chips
      delete this.chips[i];
      switch (i) {
        case "stock":
          this.stock = false;
        ...
        default:
          ...
          Object.keys(obj).map(function(Key) {
            // searching in store to delete from
            var val = obj[Key];
            if (val.indexOf(i) >= 0) {
    ...
    clearSearch() {
      //callback for md-clear in searchbox
      this.searchInput = "";
      console.log("hh");
      this.getContents();
    },
    update_chips() {
      ...
      Object.keys(this.chips).map(Key => {
        if (Key != "search" && Key != "stock") this.chips[Key] = filters[Key];
      });
    },
    update_pager() {
      var i = 1;
